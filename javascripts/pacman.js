...
function Game() {
    ...
    this.init = function (state) {
        ...
        if (pinky == null) {
            pinky = new Ghost("pinky", 7, 5, 'images/pinky.svg', 2, 2);
            inky = new Ghost("inky", 8, 5, 'images/inky.svg', 13, 11);
            blinky = new Ghost("blinky", 9, 5, 'images/blinky.svg', 13, 0);
            clyde = new Ghost("clyde", 10, 5, 'images/clyde.svg', 2, 11);
        } else {
...
function Ghost(name, gridPosX, gridPosY, image, gridBaseX, gridBaseY) {
    ...
    this.checkCollision = function () {
        ...
        if (this.dead && (this.getGridPosX() == this.startPosX / 30) && (this.getGridPosY() == this.startPosY / 30)) this.reset();
            ...
            if ((between(pacman.getCenterX(), this.getCenterX() - 10, this.getCenterX() + 10)) && (between(pacman.getCenterY(), this.getCenterY() - 10, this.getCenterY() + 10))) {
...
function pacman() {
    ...
    this.dieFinal = function () {
        ...
        if (this.lives <= 0) {
            ...
            var input = "<div id='highscore-form'></div>";
...
function doKeyDown(evt) {
    switch (evt.keyCode) {
        ...
        case 77:
            ...
            break;
        case 8:
            // Backspace pressed -> show Game Content
        case 27:
            // ESC pressed -> show Game Content
            if (!$('#playerName').is(':focus')) {
                evt.preventDefault();
                game.showContent('game-content');
            }
